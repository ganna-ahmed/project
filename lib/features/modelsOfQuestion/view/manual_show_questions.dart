...
//final String year;
  ...
  @override
  State<QuestionsForChapterManualPage> createState() =>
      _QuestionsForChapterManualPageState();
}

class _QuestionsForChapterManualPageState
    extends State<QuestionsForChapterManualPage> {
  ...
  Future<void> _loadQuestions() async {
    ...
    try {
      // Modified API call to match the second code's structure
      ...
      if (response.statusCode == 200) {
        ...
        for (var question in allQuestions) {
          String type = question['type'].toString().toLowerCase();
          if (type == 'mcq') {
        ...
        setState(() {
          ...
          essayQuestions = essayList;
          multiChoiceQuestions = multiList;
  ...
  Future<void> _addToExam() async {
    ...
    try {
      // Format the data similar to second code
      final formattedData = {
        ...
        'multi': selectedQuestions['multi']!.map((q) {
          return {
            'passage': q['paragraph'] ?? q['passage'],
            'questions': (q['questions'] as List).map((subQ) {
              return {
                'question': subQ['question'],
                'options': subQ['choices'] ?? subQ['options'],
              };
            }).toList(),
          };
        }).toList(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Questions from Chapter Maual'),
        backgroundColor: AppColors.ceruleanBlue,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if ((question['options'] ?? question['choices']) != null &&
              (question['options'] ?? question['choices']) is List) ...[
            SizedBox(height: 8.h),
            ...List.generate(
